...
#Region "Funciones"
    Public Sub buscarAlumno(ByVal codigo As Integer)
        Try
            Dim obtenerAlumno = (From x In modelo.students Where x.state = "activo" And x.id = codigo Select fullName = (x.first_name + " " + x.last_name), x.in_charge).ToList
    ...
    Public Sub buscarMensualidad(ByVal codigo As Integer)
        Try
            Dim obtenerIdInscripcion = (From x In modelo.inscripcions Where x.id_student = codigo Select x).ToList
            idInscripcion = obtenerIdInscripcion(0).id
            Dim obtenerMensualidad = (From x In modelo.shares Where x.id_inscripcion = idInscripcion And x.balance <> 0 Select x).First
            fecha = obtenerMensualidad.date_share
            idMensualidad = obtenerMensualidad.id
            ...
            End If
            txtMes.Text = mes
            txtCuota.Text = obtenerMensualidad.amount
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
    End Sub
